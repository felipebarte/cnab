...
// ✅ NOVO: Importar parser específico multi-state
import { CNAB240SpecificParser } from './specific-parser/index.js';
...
export class CNAB240Parser {
  /**
   * Inicializar parser específico (opcional)
   * @param {Object} options - Opções de configuração
   * @returns {CNAB240SpecificParser} Instância do parser específico
   */
  static createSpecificParser(options = {}) {
    return new CNAB240SpecificParser(options);
  }

  /**
   * Parser completo que processa arquivo inteiro com opção de usar parser específico
   * @param {string} fileContent - Conteúdo completo do arquivo CNAB240
   * @param {Object} options - Opções de processamento
   * @returns {Object} Resultado do processamento
   */
  static async parseFileWithSpecificParser(fileContent, options = {}) {
    try {
      // 1. Primeiro, processar com o parser tradicional
      const lines = fileContent.split('\n').filter(line => line.trim().length > 0);
      const traditionalResult = this.parseFile(lines);

      // 2. Se habilitado, usar parser específico
      if (options.enableSpecificParser) {
        const specificParser = this.createSpecificParser(options.specificParserOptions);
        const specificResult = await specificParser.processFile(fileContent, traditionalResult);

        return {
          success: true,
          hybrid: true,
          tradicional: traditionalResult,
          especifico: specificResult,
          // O resultado específico já contém o merge inteligente
          resultado: specificResult
        };
      }

      // 3. Retornar apenas resultado tradicional
      return {
        success: true,
        hybrid: false,
        tradicional: traditionalResult,
        resultado: traditionalResult
      };

    } catch (error) {
      console.error('Erro no parser híbrido:', error);

      // Fallback para parser tradicional em caso de erro
      try {
        const lines = fileContent.split('\n').filter(line => line.trim().length > 0);
        const fallbackResult = this.parseFile(lines);
