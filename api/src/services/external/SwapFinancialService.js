...
class SwapFinancialService extends ExternalAPIService {
  constructor(options = {}) {
    // Obter configuração centralizada do swapConfig.js
    const swapServiceConfig = getSwapServiceConfig();
    ...
    const defaultOptions = {
      ...
      baseURL: swapServiceConfig.baseURL
    };

    super({ ...defaultOptions, ...options });

    // Configurações específicas da Swap Financial (via config centralizado)
    this.environment = swapServiceConfig.environment;
    this.clientId = swapServiceConfig.clientId;
    this.clientSecret = swapServiceConfig.clientSecret;
    this.apiKey = swapServiceConfig.apiKey;
    this.accountId = options.accountId || process.env.SWAP_ACCOUNT_ID; // ID da conta para pagamentos

    // Usar configuração centralizada para baseURL
    this.baseURL = swapServiceConfig.baseURL;
